...
- **US Patent 7,110,538 has expired:** Bitcoin transactions are secured using
  ...
  speed up Bitcoin's initial sync.

    In 2011, [Hal Finney described][finney endomorphism] a way to more
    ...
    preserves all relationships between points). By using this endomorphism,
    the multiplication can effectively be broken into two parts, calculated separately,
    and then recombined to arrive at the solution. Doing this can reduce the
    number of expensive computations by up to 33%. Finney wrote a proof-of-concept
    implementation of the endomorphism, which he claimed sped up signature
    verification by around 25%.

    Pieter Wuille separately implemented the efficient endomorphism algorithm
    in the [libsecp][] library, which is used to verify signatures in Bitcoin Core.
    However, the algorithm was encumbered by [U.S. Patent 7,110,538][endomorphism
    patent] and so the implementation has not previously been
    distributed to users. On September 25, the patent expired, meaning that the
    efficient endomorphism algorithm can be included in the distributed binaries of
    Bitcoin Core. [A PR has been opened in the libsecp repo][endomorphism PR] to
    always use the efficient endomorphism algorithm, which is expected to decrease
